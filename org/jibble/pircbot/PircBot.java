 ...
 * 
 * This software is dual-licensed, allowing you to choose between the GNU General Public License
 * (GPL) and the www.jibble.org Commercial License. Since the GPL may be too restrictive for use in
 * a proprietary application, a commercial license is also provided. Full license information can be
 ...
 * <p>
 * It provides an event-driven architecture to handle common IRC events, flood protection, DCC
 * support, ident support, and more. The comprehensive logfile format is suitable for use with pisg
 * to generate channel statistics.
 * <p>
 * Methods of the PircBot class can be called to send events to the IRC server that it connects to.
 * For example, calling the sendMessage method will send a message to a channel or user on the IRC
 * server. Multiple servers can be supported using multiple instances of PircBot.
 * <p>
 * To perform an action when the PircBot receives a normal message from the IRC server, you would
 * override the onMessage method defined in the PircBot class. All on<i>XYZ</i> methods in the
 * PircBot class are automatically called when the event <i>XYZ</i> happens, so you would override
 * these if you wish to do something when it does happen.
 * <p>
 * Some event methods, such as onPing, should only really perform a specific function (i.e. respond
 * to a PING from the server). For your convenience, such methods are already correctly implemented
 * in the PircBot and should not normally need to be overridden. Please read the full documentation
 * for each method to see which ones are already implemented by the PircBot class.
 * <p>
 * Please visit the PircBot homepage at <a href="http://www.jibble.org/pircbot.php"
 * >http://www.jibble.org/pircbot.php</a> for full revision history, a beginners guide to creating
 * your first PircBot and a list of some existing Java IRC bots and clients that use the PircBot
 * framework.
 * 
...
public abstract class PircBot implements ReplyConstants {
        ...
        /**
         * Constructs a PircBot with the default settings. Your own constructors in classes which
         * extend the PircBot abstract class should be responsible for changing the default settings
         * if required.
         */
        public PircBot() {}

        /**
         * Returns the last SocketFactory that we used to connect to an IRC server. This does not
         * imply that the connection attempt to the server was successful (we suggest you look at
         * the onConnect method). A value of null is returned if the PircBot has never tried to
         * connect to a server using a SocketFactory.
         * 
         * @return The last SocketFactory that we used when connecting to an IRC server. Returns
         *         null if we have not previously connected using a SocketFactory.
         */
         ...
         * appropriate port number and password will be used. This method will throw an IrcException
         * if we have never connected to an IRC server previously.
         * 
         * @since PircBot 0.9.9
         * 
         * @throws IOException if it was not possible to connect to the server.
         ...
         * Providing the PircBot was connected to an IRC server, the onDisconnect() will be called
         * as soon as the disconnection is made by the server.
         * 
        ...
        /**
         * When you connect to a server and your nick is already in use and this is set to true, a
         * new nick will be automatically chosen. This is done by adding numbers to the end of the
         * nick until an available nick is found.
         * 
         ...
         * determine the user's identity. A few IRC servers will not allow you to connect unless
         ...
         * <p>
         * Calling this method starts up an ident server which will respond with the login provided
         * by calling getLogin() and then shut down immediately. It will also be shut down if it has
         * not been contacted within 60 seconds of creation.
         * <p>
         * If you require an ident response, then the correct procedure is to start the ident server
         * and then connect to the IRC server. The IRC server may then contact the ident server to
         * get the information it needs.
         * <p>
         * The ident server will fail to start if there is already an ident server running on port
         * 113, or if you are running as an unprivileged user who is unable to create a server
         * socket on that port number.
         * <p>
         * If it is essential for you to use an ident server when connecting to an IRC server, then
         * make sure that port 113 on your machine is visible to the IRC server so that it may
         * contact the ident server.
         * 
         * @since PircBot 0.9c
         */
        ...
        /**
         * Quits from the IRC server. Providing we are actually connected to an IRC server, the
         * onDisconnect() method will be called as soon as the IRC server disconnects us.
         */
        ...
        /**
         * Quits from the IRC server with a reason. Providing we are actually connected to an IRC
         * server, the onDisconnect() method will be called as soon as the IRC server disconnects
         * us.
         * 
        ...
        /**
         * Sends a raw line to the IRC server as soon as possible, bypassing the outgoing message
         * queue.
         * 
        ...
        /**
         * Sends a message to a channel or a private message to a user. These messages are added to
         * the outgoing message queue and sent at the earliest possible opportunity.
         * <p>
         * Some examples: -
         * 
         * <pre>
         ...
         * commands are "PING <number>", "FINGER", "VERSION", etc. For example, if you wish to
         * request the version of a user called "Dave", then you would call
         * <code>sendCTCPCommand("Dave", "VERSION");</code>. The type of response to such commands
         * is largely dependant on the target client software.
         * 
         * @since PircBot 0.9.5
         * 
        ...
        /**
         * Attempt to change the current nick (nickname) of the bot when it is connected to an IRC
         * server. After confirmation of a successful nick change, the getNick method will return
         * the new nick.
         * 
        ...
        /**
         * Identify the bot with NickServ, supplying the appropriate password. Some IRC Networks
         * (such as freenode) require users to <i>register</i> and <i>identify</i> with NickServ
         * before they are able to send private messages to other users, thus reducing the amount of
         * spam. If you are using an IRC network where this kind of policy is enforced, you will
         * need to make your bot <i>identify</i> itself to NickServ before you can send private
         * messages. Assuming you have already registered your bot's nick with NickServ, this method
         * can be used to <i>identify</i> with the supplied password. It usually makes sense to
         * identify with NickServ immediately after connecting to a server.
         * <p>
         * This method issues a raw NICKSERV command to the server, and is therefore safer than the
         * alternative approach of sending a private message to NickServ. The latter approach is
         * considered dangerous, as it may cause you to inadvertently transmit your password to an
         * untrusted party if you connect to a network which does not run a NickServ service and
         * where the untrusted party has assumed the nick "NickServ". However, if your IRC network
         * is only compatible with the private message approach, you may typically identify like so:
         * 
         * <pre>
         ...
         * require the bot to have operator status on the channel. For example, if the bot has
         * operator status, we can grant operator status to "Dave" on the #cs channel by calling
         * setMode("#cs", "+o Dave"); An alternative way of doing this would be to use the op
         * method.
         * 
         * @param channel The channel on which to perform the mode change.
         * @param mode The new mode to apply to the channel. This may include zero or more arguments
         *        if necessary.
         * 
        ...
        /**
         * Sends an invitation to join a channel. Some channels can be marked as "invite-only", so
         * it may be useful to allow a bot to invite people into it.
         * 
        ...
        /**
         * Bans a user from a channel. An example of a valid hostmask is "*!*compu@*.18hp.net". This
         * may be used in conjunction with the kick method to permanently remove a user from a
         * channel. Successful use of this method may require the bot to have operator status
         * itself.
         * 
        ...
        /**
         * Grants operator privilidges to a user on a channel. Successful use of this method may
        ...
        /**
         * Removes voice privilidges from a user on a channel. Successful use of this method may
         * require the bot to have operator status itself.
         * 
        ...
        /**
         * Issues a request for a list of all channels on the IRC server. When the PircBot receives
         * information for each channel, it will call the onChannelInfo method, which you will need
         * to override if you want it to do anything useful.
         * 
         ...
         * to override if you want it to do anything useful.
         * <p>
         * Some IRC servers support certain parameters for LIST requests. One example is a parameter
         * of ">10" to list only those channels that have more than 10 users in them. Whether these
         * parameters are supported or not will depend on the IRC server software.
         * 
         ...
         * <p>
         * You may throttle the speed of this file transfer by calling the setPacketDelay method on
         * the DccFileTransfer that is returned.
         * <p>
         ...
         * @param timeout The number of milliseconds to wait for the recipient to acccept the file
         ...
         * successfully made, then a DccChat object is returned by this method. If the connection is
         * not made within the time limit specified by the timeout value, then null is returned.
         * <p>
         * It is <b>strongly recommended</b> that you call this method within a new Thread, as it
         ...
         * @param timeout The number of milliseconds to wait for the recipient to accept the chat
        ...
        /**
         * Attempts to accept a DCC CHAT request by a client. Please use the onIncomingChatRequest
         ...
         * override this method if you wish to do something else with log entries. Each line in the
         * log begins with a number which represents the logging time (as the number of milliseconds
         * since the epoch). This timestamp and the following log entry are separated by a single
         * space character, " ". Outgoing messages are distinguishable by a log entry that has ">>>"
         ...
         * <p>
         * This implementation of the method will only cause log entries to be output if the PircBot
        ...
        /**
         * This method handles events when any line of text arrives from the server, then calling
         * the appropriate method in the PircBot. This method is protected and only called by the
         ...
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         ...
         * This may happen if the PircBot quits from the server, or if the connection is
         * unexpectedly lost.
         * <p>
         * Disconnection from the IRC server is detected immediately if either we or the server
         * close the connection normally. If the connection to the server is lost, but neither we
         * nor the server have explicitly closed the connection, then it may take a few minutes to
         * detect (this is commonly referred to as a "ping timeout").
         * <p>
        ...
        /**
         * This method is called by the PircBot when a numeric response is received from the IRC
         * server. We use this method to allow PircBot to process various responses from the server
         * before then passing them on to the onServerResponse method.
         * <p>
         * Note that this method is private and should not appear in any of the javadoc generated
         * documenation.
         * 
         ...
         * <p>
         * Numerics in the range from 001 to 099 are used for client-server connections only and
         * should never travel between servers. Replies generated in response to commands are found
         * in the range from 200 to 399. Error replies are found in the range from 400 to 599.
         * <p>
         * For example, we can use this method to discover the topic of a channel when we join it.
         * If we join the channel #test which has a topic of &quot;I am King of Test&quot; then the
         * response will be &quot; <code>PircBot #test :I Am King of Test</code>&quot; with a code
         * of 332 to signify that this is a topic. (This is just an example - note that overriding
         * the <code>onTopic</code> method is an easier way of finding the topic for a channel).
         * Check the IRC RFC for the full list of other command response codes.
         * <p>
         * PircBot implements the interface ReplyConstants, which contains contstants that you may
         * find useful here.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         ...
         * channel. The PircBot collects this information and calls this method as soon as it has
         * the full list.
         * <p>
         * To obtain the nick of each user in the channel, call the getNick() method on each User
         * object in the array.
         * <p>
         * At a later time, you may call the getUsers method to obtain an up to date list of the
         ...
         * by typing "/me goes shopping" in most IRC clients.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         * 
        ...
        /**
         * This method is called whenever someone (possibly us) changes nick on any of the channels
         * that we are on.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
        ...
        /**
         * This method is called whenever someone (possibly us) is kicked from any of the channels
         * that we are in.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
        ...
        /**
         * This method is called whenever a user sets the topic, or when PircBot joins a new channel
         * and discovers its topic.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         * 
         ...
         * @param changed True if the topic has just been changed, false if the topic was already
         *        there.
         * 
         */
        ...
        /**
         * After calling the listChannels() method in PircBot, the server will start to send us
         * information about each channel on the server. You may override this method in order to
         * receive the information about each channel as soon as it is received.
         * <p>
         * Note that certain channels, such as those marked as hidden, may not appear in channel
         * listings.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         ...
         * appropriate onOp, onDeop, etc method before finally calling the override-able onMode
         * method.
         * <p>
         * Note that this method is private and is not intended to appear in the javadoc generated
         * documentation.
         * 
         ...
         * <p>
         * You may find it more convenient to decode the meaning of the mode string by overriding
         * the onOp, onDeOp, onVoice, onDeVoice, onChannelKey, onDeChannelKey, onChannelLimit,
         * onDeChannelLimit, onChannelBan or onDeChannelBan methods as appropriate.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         * 
         ...
         * <p>
         * This is a type of mode change and is also passed to the onMode method in the PircBot
         * class.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         ...
         * join that channel if they know the key. Channel keys are sometimes referred to as
         * passwords.
         * <p>
         * This is a type of mode change and is also passed to the onMode method in the PircBot
         * class.
         ...
         * prevents any user with a matching hostmask from joining the channel. For this reason,
         * most bans are usually directly followed by the user being kicked :-)
         * <p>
         * This is a type of mode change and is also passed to the onMode method in the PircBot
         * class.
         * <p>
         ...
         * operators in a channel may change the topic.
         * <p>
         * This is a type of mode change and is also passed to the onMode method in the PircBot
         * class.
         * <p>
        ...
        /**
         * Called when a channel is set to 'invite only' mode. A user may only join the channel if
         * they are invited by someone who is already in the channel.
         * <p>
         * This is a type of mode change and is also passed to the onMode method in the PircBot
         * class.
         * <p>
        ...
        /**
         * Called when a channel is set to 'moderated' mode. If a channel is moderated, then only
         * users who have been 'voiced' or 'opped' may speak or change their nicks.
         * <p>
         * This is a type of mode change and is also passed to the onMode method in the PircBot
         * class.
         * <p>
        ...
        /**
         * This method used to be called when a DCC SEND request was sent to the PircBot. Please use
         * the onIncomingFileTransfer method to receive files, as it has better functionality and
         * supports resuming.
         * 
         * @deprecated As of PircBot 1.2.0, use {@link #onIncomingFileTransfer(DccFileTransfer)}
         */
        ...
        /**
         * This method used to be called when a DCC CHAT request was sent to the PircBot. Please use
         * the onIncomingChatRequest method to accept chats, as it has better functionality.
         * 
         * @deprecated As of PircBot 1.2.0, use {@link #onIncomingChatRequest(DccChat)}
         */
        ...
        /**
         * This method is called whenever a DCC SEND request is sent to the PircBot. This means that
         * a client has requested to send a file to us. This abstract implementation performs no
         * action, which means that all DCC SEND requests will be ignored by default. If you wish to
         * receive the file, then you may override this method and call the receive method on the
         ...
         * </pre>
         * <p>
         * <b>Warning:</b> Receiving an incoming file transfer will cause a file to be written to
         * disk. Please ensure that you make adequate security checks so that this file does not
         * overwrite anything important!
         * <p>
         * Each time a file is received, it happens within a new Thread in order to allow multiple
         * files to be downloaded by the PircBot at the same time.
         * <p>
         * If you allow resuming and the file already partly exists, it will be appended to instead
         * of overwritten. If resuming is not enabled, the file will be overwritten if it already
         * exists.
         * <p>
         * You can throttle the speed of the transfer by calling the setPacketDelay method on the
         * DccFileTransfer object, either before you receive the file or at any moment during the
         * transfer.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
        ...
        /**
         * This method gets called when a DccFileTransfer has finished. If there was a problem, the
         * Exception will say what went wrong. If the file was sent successfully, the Exception will
         * be null.
         * <p>
         * Both incoming and outgoing file transfers are passed to this method. You can determine
         * the type by calling the isIncoming or isOutgoing methods on the DccFileTransfer object.
         * 
         * @since PircBot 1.2.0
         * 
         * @param transfer The DccFileTransfer that has finished.
         * @param e null if the file was transfered successfully, otherwise this will report what
         *        went wrong.
         * 
         * @see DccFileTransfer
         * 
        ...
        /**
         * This method will be called whenever a DCC Chat request is received. This means that a
         * client has requested to chat to us directly rather than via the IRC server. This is
         * useful for sending many lines of text to and from the bot without having to worry about
         * flooding the server or any operators of the server being able to "spy" on what is being
         * said. This abstract implementation performs no action, which means that all DCC CHAT
         * requests will be ignored by default.
         * <p>
         * If you wish to accept the connection, then you may override this method and call the
         * accept() method on the DccChat object, which connects to the sender of the chat request
         * and allows lines to be sent to and from the bot.
         * <p>
         ...
         * </pre>
         * 
         * Each time this method is called, it is called from within a new Thread so that multiple
         * DCC CHAT sessions can run concurrently.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         * 
         ...
         * responds with the PircBot's _version string, so if you override this method, be sure to
         ...
         * <p>
         * This abstract implementation responds correctly, so if you override this method, be sure
         * to either mimic its functionality or to call super.onPing(...);
         * 
         ...
         * <p>
         * This sends back a correct response, so if you override this method, be sure to either
         * mimic its functionality or to call super.onServerPing(response);
         * 
         ...
         * <p>
         * This abstract implementation responds correctly, so if you override this method, be sure
         * to either mimic its functionality or to call super.onTime(...);
         * 
         ...
         * <p>
         * This abstract implementation responds correctly, so if you override this method, be sure
         * to either mimic its functionality or to call super.onFinger(...);
         * 
        ...
        /**
         * This method is called whenever we receive a line from the server that the PircBot has not
         * been programmed to recognise.
         * <p>
         * The implementation of this method in the PircBot abstract class performs no actions and
         * may be overridden as required.
         * 
        ...
        /**
         * Sets the verbose mode. If verbose mode is set to true, then log entries will be printed
         * to the standard output. The default value is false and will result in no output. For
         * general development, we strongly recommend setting the verbose mode to true.
         * 
        ...
        /**
         * Sets the name of the bot, which will be used as its nick when it tries to join an IRC
         * server. This should be set before joining any servers, otherwise the default nick will be
         * used. You would typically call this method from the constructor of the class that extends
         * PircBot.
         * <p>
        ...
        /**
         * Sets the internal nick of the bot. This is only to be called by the PircBot class in
         * response to notification of nick changes that apply to us.
         * 
        ...
        /**
         * Gets the name of the PircBot. This is the name that will be used as as a nick when we try
         * to join servers.
         * 
        ...
        /**
         * Returns the current nick of the bot. Note that if you have just changed your nick, this
         * method will still return the old nick until confirmation of the nick change is received
         * from the server.
         * <p>
        ...
        /**
         * Returns whether or not the PircBot is currently connected to a server. The result of this
         * method should only act as a rough guide, as the result may not be valid by the time you
         * act upon it.
         * 
        ...
        /**
         * Sets the number of milliseconds to delay between consecutive messages when there are
         * multiple messages waiting in the outgoing message queue. This has a default value of
         * 1000ms. It is a good idea to stick to this default value, as it will prevent your bot
         * from spamming servers and facing the subsequent wrath! However, if you do need to change
         * this delay value (<b>not recommended</b>), then this is the method to use.
         * 
        ...
        /**
         * Returns the number of milliseconds that will be used to separate consecutive messages to
         * the server from the outgoing message queue.
         * 
         ...
         * Hence, there is currently no option to change this value in PircBot. All lines greater
         * than this length will be truncated before being sent to the IRC server.
         * 
        ...
        /**
         * Gets the number of lines currently waiting in the outgoing message Queue. If this returns
         * 0, then the Queue is empty and any new message is likely to be sent to the IRC server
         * immediately.
         * 
         * @since PircBot 0.9.9
         * 
        ...
        /**
         * Returns the name of the last IRC server the PircBot tried to connect to. This does not
         * imply that the connection attempt to the server was successful (we suggest you look at
         * the onConnect method). A value of null is returned if the PircBot has never tried to
         * connect to a server.
         * 
        ...
        /**
         * Returns the port number of the last IRC server that the PircBot tried to connect to. This
         * does not imply that the connection attempt to the server was successful (we suggest you
         * look at the onConnect method). A value of -1 is returned if the PircBot has never tried
        ...
        /**
         * Returns the last password that we used when connecting to an IRC server. This does not
         * imply that the connection attempt to the server was successful (we suggest you look at
         * the onConnect method). A value of null is returned if the PircBot has never tried to
         * connect to a server using a password.
         * 
         * @since PircBot 0.9.9
         * 
         * @return The last password that we used when connecting to an IRC server. Returns null if
         *         we have not previously connected using a password.
         */
        ...
        /**
         * A convenient method that accepts an IP address represented by a byte[] of size 4 and
         * returns this as a long representation of the same IP address.
         * 
         * @since PircBot 0.9.4
         * 
         ...
         * If set to null, then the platform's default charset is used. You should only use this
         * method if you are trying to send text to an IRC server in a different charset, e.g.
         * "GB2312" for Chinese encoding. If a PircBot is currently connected to a server, then it
         * must reconnect before this change takes effect.
         * 
         * @since PircBot 1.0.4
         * 
        ...
        /**
         * Returns the encoding used to send and receive lines from the IRC server, or null if not
         * set. Use the setEncoding method to change the encoding charset.
         * 
         * @since PircBot 1.0.4
         * 
        ...
        /**
         * Returns the InetAddress used by the PircBot. This can be used to find the I.P. address
         * from which the PircBot is connected to a server.
         * 
         * @since PircBot 1.4.4
         * 
        ...
        /**
         * Sets the InetAddress to be used when sending DCC chat or file transfers. This can be very
         * useful when you are running a bot on a machine which is behind a firewall and you need to
         * tell receiving clients to connect to a NAT/router, which then forwards the connection.
         * 
         * @since PircBot 1.4.4
         * 
        ...
        /**
         * Returns the InetAddress used when sending DCC chat or file transfers. If this is null,
         * the default InetAddress will be used.
         * 
         * @since PircBot 1.4.4
         * 
        ...
        /**
         * Returns the set of port numbers to be used when sending a DCC chat or file transfer. This
         * is useful when you are behind a firewall and need to set up port forwarding. The array of
         * port numbers is traversed in sequence until a free port is found to listen on. A DCC
         * tranfer will fail if all ports are already in use. If set to null, <i>any</i> free port
         * number will be used.
         * 
         * @since PircBot 1.4.4
         * 
         * @return An array of port numbers that PircBot can use to send DCC transfers, or null if
         *         any port is allowed.
         */
         ...
         * forwarding. The array of port numbers is traversed in sequence until a free port is found
         * to listen on. A DCC tranfer will fail if all ports are already in use. If set to null,
         * <i>any</i> free port number will be used.
         * 
         * @since PircBot 1.4.4
         * 
         * @param ports The set of port numbers that PircBot may use for DCC transfers, or null to
         *        let it use any free port (default).
         * 
         */
         ...
         * PircBot. This may be useful if you are writing a multiple server IRC bot that uses more
         * than one instance of PircBot.
         * 
         * @since PircBot 0.9.9
         * 
        ...
        /**
         * Returns the hashCode of this PircBot. This method can be called by hashed collection
         * classes and is useful for managing multiple instances of PircBots in such collections.
         * 
         * @since PircBot 0.9.9
         * 
         ...
         * purposes, particularly if you are using more than one PircBot instance to achieve
         * multiple server connectivity. The format of this String may change between different
         * versions of PircBot but is currently something of the form <code>
         ...
         * <ul>
         * <li>This method may not return a full list of users if you call it before the complete
         * nick list has arrived from the IRC server.</li>
         * <li>If you wish to find out which users are in a channel as soon as you join it, then you
         * should override the onUserList method instead of calling this method, as the onUserList
         * method is only called as soon as the full user list has been received.</li>
         * <li>This method will return immediately, as it does not require any interaction with the
         * IRC server.</li>
         ...
         * immediately after joining a new channel, the new channel may not appear in this array as
         * it is not possible to tell if the join was successful until a response is received from
         * the IRC server.
         * 
         * @since PircBot 1.0.0
         * 
        ...
        /**
         * Disposes of all thread resources used by this PircBot. This may be useful when writing
         * bots or clients that use multiple servers (and therefore multiple PircBot instances) or
         * when integrating a PircBot with an existing program.
         * <p>
         * Each PircBot runs its own threads for dispatching messages from its outgoing message
         * queue and receiving messages from the server. Calling dispose() ensures that these
         * threads are stopped, thus freeing up system resources and allowing the PircBot object to
         * be garbage collected if there are no other references to it.
         * <p>
         * Once a PircBot object has been disposed, it should not be used again. Attempting to use a
         * PircBot that has been disposed may result in unpredictable behaviour.
         * 
         * @since 1.2.2
         */
        ...
        /**
         * Add a user to the specified channel in our memory. Overwrite the existing entry if it
         * exists.
         */
